 ...
 * Depending on the setting chosen in {@link SpeechRecognitionSettings}, it either returns a
...
public class SpeechRecognitionService extends RecognitionService {
    ...
    protected void onStartListening(Intent recognizerIntent, Callback listener) {
        ...
        SharedPreferences prefs = getSharedPreferences(
                SpeechRecognitionSettings.SHARED_PREFERENCES_NAME,
                Context.MODE_PRIVATE);
        
        String resultType = prefs.getString(
                SpeechRecognitionSettings.PREF_KEY_RESULTS_TYPE,
                String.valueOf(SpeechRecognitionSettings.RESULT_TYPE_LETTERS));
        int resultTypeInt = Integer.parseInt(resultType);
        
        if (resultTypeInt == SpeechRecognitionSettings.RESULT_TYPE_LETTERS) {
            results.add("a");
            results.add("b");
            results.add("c");            
        } else if (resultTypeInt == SpeechRecognitionSettings.RESULT_TYPE_NUMBERS) {
            results.add("1");
            results.add("2");
            results.add("3");
        }
